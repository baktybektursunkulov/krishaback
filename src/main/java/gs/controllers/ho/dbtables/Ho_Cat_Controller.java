...
public class Ho_Cat_Controller {
  ...
  public ResponseEntity<List<Offer_previewResponse>> preview_sell_rent(@Valid @RequestParam Integer id, @Valid @RequestParam Integer parent_id, @Valid @RequestParam Integer page, @Valid @RequestParam Integer limit, HttpServletRequest httpServletRequest) throws RuntimeException {
    ...
    while (ho_ads.size() < limit && ho_cat.size() > k) {
      if (ho_ads.size() < limit) {
        ho_ads.addAll(ho_ad_Service.pagination(ho_cat.get(k), limit - ho_ads.size(), 0));
        k++;
      }
    }
    int i = 0;
    while (i < ho_ads.size()) {
      ...
      if (!c_tbl_rec_img_moder_list_.isEmpty()) {
        ...
        Ho_House_Shop_Type shop_type = ho_house_shop_type_service.find_by_id(ho_ad.getHo_house_shop_type());
        String s_typ = "";
        if (ho_ad.getHo_house_shop_type() != null) {
          s_typ = shop_type.getName();
        }
        Ho_House_Indus_Base_Type indus_type = ho_house_indus_base_type_service.find_by_id(ho_ad.getHo_house_indus_base_type());
        String i_typ = "";
        if (ho_ad.getHo_house_indus_base_type() != null) {
          i_typ = indus_type.getName();
        }
        String t = "";
        if (ho_ad.getRoom_cnt() != null && ho_ad.getHo_cat() != 15 && ho_ad.getHo_cat() != 26) {
          t = String.valueOf(ho_ad.getRoom_cnt());
        }

        String title = "";
        if (ho_ad.getTitle() != null) {
          title += ho_ad.getTitle();
        }
        String Ho_house_rent_period = ho_house_rent_period_service.find_by_id(ho_ad.getHo_house_rent_period());
        String rent_period = "";
        ...
        if (ho_ad.getHo_cat() == 13) {
          ...
          total_area = " " + ho_ad.getTotal_area().intValue() + " м²";
        } else if (i_typ != "" && ho_ad.getTerritory_area() != null) {
          total_area = " " + ho_ad.getTerritory_area().intValue() + " м²";
        }
        if (ho_ad.getHo_cat() == 16 || ho_ad.getHo_cat() == 27 || ho_ad.getHo_cat() == 15 || ho_ad.getHo_cat() == 26) {
          total_area = " " + ho_ad.getTotal_area().intValue() + " м²";
        }
        String floor = "";
        ...
        if (ho_ad.getFloor() != null) {
          ...
          if (ho_ad.getMax_floor() != null) {
            floor += "/" + ho_ad.getMax_floor();
          }
          floor += floor_ru;
        }
